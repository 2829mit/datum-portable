            ...
            <div className="mb-[45px]">
                ...
                <div className="space-y-6">
                  {SAFETY_UPGRADE_OPTIONS.filter(opt => ['advanced-skid', 'backup-du', 'sampling-kit', 'fuel-monitoring-system'].includes(opt.id) && !(selectedTank === '30kl' && opt.id === 'advanced-skid')).map(option => {
                    return (
                      <button
                        key={option.id}
            ...
            <div className="mb-[45px]">
                ...
                <div className="space-y-6">
                  {SAFETY_UPGRADE_OPTIONS.filter(opt => ['crash-barrier', 'fire-suppression'].includes(opt.id)).map(option => {
                    return (
            ...
            {/* 7b. Secure Fueling System */}
            <div className="mb-[45px]">
                <h2 className="font-medium text-[20px] leading-[28px] text-[#171A20] mb-3 text-center">Secure Fueling System</h2>
                <div className="space-y-6">
                  {SAFETY_UPGRADE_OPTIONS.filter(opt => ['secure-fueling-1', 'secure-fueling-2'].includes(opt.id)).map(option => {
                    return (
                      <button
                        key={option.id}
                        onClick={() => onSafetyUpgradeToggle(option)}
