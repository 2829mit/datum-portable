...
interface ConfiguratorProps {
  ...
  setSelectedProduct?: (prod: 'rfd-portable' | 'rfd-z' | 'portable-o') => void;
  ...
  setPortableOUgTanks?: (val: 0 | 1 | 2) => void;
  portableODuQuantity?: 0 | 1 | 2 | 3;
  setPortableODuQuantity?: (val: 0 | 1 | 2 | 3) => void;
  portableOFuelLevelSensors?: number;
  setPortableOFuelLevelSensors?: (val: number) => void;
  portableORfidActive?: number;
  setPortableORfidActive?: (val: number) => void;
  portableORfidPassive?: number;
  setPortableORfidPassive?: (val: number) => void;
  portableOIotController?: boolean;
  setPortableOIotController?: (val: boolean) => void;
  portableODatumF?: boolean;
  setPortableODatumF?: (val: boolean) => void;
  portableOInstallation?: boolean;
  setPortableOInstallation?: (val: boolean) => void;
  portableODuBrand?: string;
  setPortableODuBrand?: (val: string) => void;
  portableODuNozzle?: string;
  setPortableODuNozzle?: (val: string) => void;
}
...
const Configurator: React.FC<ConfiguratorProps> = ({
  ...
  setPortableODuQuantity,
  portableOFuelLevelSensors = 0,
  setPortableOFuelLevelSensors,
  portableORfidActive = 0,
  setPortableORfidActive,
  portableORfidPassive = 0,
  setPortableORfidPassive,
  portableOIotController = false,
  setPortableOIotController,
  portableODatumF = false,
  setPortableODatumF,
  portableOInstallation = false,
  setPortableOInstallation,
  portableODuBrand = '',
  setPortableODuBrand,
  portableODuNozzle = '',
  setPortableODuNozzle,
}) => {
  ...
  if (selectedProduct === 'portable-o') {
    if (portableOUgTanks > 0) pricingItems.push({ name: 'Underground Tanks', price: 0 });
    if (portableODuQuantity > 0) pricingItems.push({ name: 'Dispensing Unit', price: 0 });
    if (portableOFuelLevelSensors > 0) pricingItems.push({ name: `Fuel Level Sensors (${portableOFuelLevelSensors} nos)`, price: portableOFuelLevelSensors * (89999/36) * multiplier });
    if (portableORfidActive > 0) pricingItems.push({ name: `RFID Active Reader (${portableORfidActive} nos)`, price: portableORfidActive * (79999/36) * multiplier });
    if (portableORfidPassive > 0) pricingItems.push({ name: `RFID Passive Tags (${portableORfidPassive} nos)`, price: portableORfidPassive * (999/36) * multiplier });
    if (portableOIotController) pricingItems.push({ name: 'IOT Controller', price: (99999/36) * multiplier });
    if (portableODatumF) pricingItems.push({ name: 'DATUM F', price: (899999/36) * multiplier });
    if (portableOInstallation) pricingItems.push({ name: 'Installation & Commissioning', price: (49999/36) * multiplier });
    if (portableODuBrand && portableODuNozzle) {
        let p = 0;
        if (portableODuNozzle === 'Single Nozzle') p = 129999;
        if (portableODuNozzle === 'Double Nozzle') p = 249999;
        pricingItems.push({ name: `DU Brand (${portableODuBrand}) - ${portableODuNozzle}`, price: (p/36) * multiplier });
    }
  } else {
    pricingItems = [
    ...
    if (rfidTagsQuantity > 0) {
      pricingItems.push({
        ...
        price: (rfidTagsQuantity * (999 / 36)) * multiplier
      });
    }
  }
        ...
        <div className="pb-6 md:pb-10">
          ...
          <div className="px-6 md:px-10">
            ...
            <div className="mb-[45px]">
              <h2 className="font-medium text-[20px] leading-[28px] text-[#171A20] mb-3 text-center">Select Product</h2>
                  <div className="flex gap-4">
                <button
                  onClick={() => setSelectedProduct?.('rfd-portable')}
                  className={`flex-1 py-4 px-2 border rounded-xl text-center transition-all duration-300 ${
                    selectedProduct !== 'portable-o' 
                      ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50 font-semibold' 
                      : 'border-gray-300 hover:border-gray-500'
                  }`}
                >
                  <p className="text-[15px] leading-[20px] text-[#171A20]">Datum Portable</p>
                </button>
                <button
                  onClick={() => setSelectedProduct?.('portable-o')}
                  className={`flex-1 py-4 px-2 border rounded-xl text-center transition-all duration-300 ${
                    selectedProduct === 'portable-o' 
                      ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50 font-semibold' 
                      : 'border-gray-300 hover:border-gray-500'
                ...
                >
                  <p className="text-[15px] leading-[20px] text-[#171A20]">Portable O</p>
                </button>
              </div>
            </div>
                ...
                <div className="space-y-6">
                  {(() => {
                    ...
                    const option2Obj = SAFETY_UPGRADE_OPTIONS.find(o => o.id === 'secure-fueling-2');
                                        ...
                                        const currentPrice = (is1Selected ? 75000 / 36 : 0) + (is2Selected ? 149000 / 36 : 0);
                        ...
                        <div className="px-4 pb-4 pt-1 border-t border-gray-200/60 mt-1">
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            ...
                            <div 
                              onClick={(e) => {
                                e.stopPropagation();
                                if (!is1Selected && is2Selected && option2Obj) onSafetyUpgradeToggle(option2Obj);
                                if (option1Obj) onSafetyUpgradeToggle(option1Obj);
                              }}
                            ...
                            >
                              ...
                              <div className="flex flex-col items-center">
                                ...
                                <span className={`text-[9px] font-medium ${is1Selected ? 'text-gray-500' : 'text-gray-400'}`}>
                                  {is1Selected ? (showPrices ? formatPrice(75000/36) : 'Included') : 'Add (+₹75,000)'}
                                </span>
                              </div>
                            </div>
                            ...
                            <div 
                              onClick={(e) => {
                                e.stopPropagation();
                                if (!is2Selected && is1Selected && option1Obj) onSafetyUpgradeToggle(option1Obj);
                            ...
                            >
                              ...
                              <div className="flex flex-col items-center">
                                ...
                                <span className={`text-[9px] font-medium ${is2Selected ? 'text-gray-500' : 'text-gray-400'}`}>
                                  {is2Selected ? (showPrices ? formatPrice(149000/36) : 'Included') : 'Add (+₹1,49,000)'}
                                </span>
                              </div>
                            </div>
                          </div>
                  ...
                  {(() => {
                    const isSecureFuelingSelected = selectedSafetyUpgrades.some(
                      o => o.id === 'secure-fueling-1' || o.id === 'secure-fueling-2'
                    );

                    return (
                      <div className={`group w-full p-4 border rounded-lg transition-all duration-300 ${
                        isSecureFuelingSelected 
                          ? 'border-gray-300 bg-white hover:border-gray-400' 
                          : 'border-gray-200 bg-gray-50/80 opacity-60 cursor-not-allowed'
                      }`}>
                        <div className="flex justify-between items-center">
                           <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                             <span className={`font-medium text-[14px] leading-[20px] ${isSecureFuelingSelected ? 'text-[#171A20]' : 'text-gray-500'}`}>RFID Tags</span>
                             {!isSecureFuelingSelected && (
                               <span className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200/80 rounded px-1.5 py-0.5 font-normal">
                                 Requires 1-Active or 2-Active Tag
                               </span>
                             )}
                           </div>
                           <div className="flex items-center gap-4">
                              <span className={`font-medium text-[14px] leading-[20px] ${isSecureFuelingSelected ? 'text-[#171A20]' : 'text-gray-400'}`}>
                                {showPrices && isSecureFuelingSelected && rfidTagsQuantity > 0 ? formatPrice(rfidTagsQuantity * (999 / 36)) : ''}
                              </span>
                              <div className={`flex items-center border rounded-md overflow-hidden ${isSecureFuelingSelected ? 'border-gray-200 bg-gray-50' : 'border-gray-200 bg-gray-100'}`}>
                                 <button 
                                   type="button"
                                   disabled={!isSecureFuelingSelected || rfidTagsQuantity <= 0}
                                   onClick={() => {
                                     if (isSecureFuelingSelected) {
                                       setRfidTagsQuantity(Math.max(0, rfidTagsQuantity - 1));
                                     }
                                   }}
                                   className={`px-3 py-1 font-bold border-r border-gray-200 transition-colors ${
                                     !isSecureFuelingSelected || rfidTagsQuantity <= 0
                                 ...
                                 <input 
                                   type="number" 
                                   min="0"
                                   disabled={!isSecureFuelingSelected}
                                   value={isSecureFuelingSelected ? rfidTagsQuantity : 0}
                                   onChange={(e) => {
                                     if (!isSecureFuelingSelected) return;
                                     setRfidTagsQuantity(Math.max(0, parseInt(e.target.value) || 0));
                                   }}
                                   className={`w-12 text-center bg-transparent text-sm font-semibold outline-none py-1 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${
                                     !isSecureFuelingSelected ? 'cursor-not-allowed text-gray-400' : 'text-gray-800'
                                   }`}
                                 />
                                 <button 
                                   type="button"
                                   ...
                                   onClick={() => {
                                     if (isSecureFuelingSelected) {
                                       setRfidTagsQuantity(rfidTagsQuantity + 1);
                                     }
                                   }}
                                   className={`px-3 py-1 font-bold border-l border-gray-200 transition-colors ${
                                     !isSecureFuelingSelected
                                       ? 'bg-gray-100 text-gray-300 cursor-not-allowed'
                                       : 'bg-white hover:bg-gray-100 text-gray-800'
                                   }`}
            ...
            {selectedProduct === 'portable-o' && (
              <>
                {/* Underground Tanks */}
                <div className="mb-[45px]">
                  <h2 className="font-medium text-[20px] leading-[28px] text-[#171A20] mb-3 text-center">Underground Tanks</h2>
                  <div className="space-y-4">
                    <button 
                        onClick={() => setPortableOUgTanks?.(portableOUgTanks === 1 ? 0 : 1)}
                        className={`group relative w-full p-4 border rounded-lg text-left transition-all duration-300 ${
                          portableOUgTanks === 1 
                            ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50' 
                            : 'border-gray-300 hover:border-gray-500'
                    ...
                    >
                      <p className="font-medium text-[14px] leading-[20px] text-[#171A20]">1 Tank</p>
                    </button>
                    <button 
                        onClick={() => setPortableOUgTanks?.(portableOUgTanks === 2 ? 0 : 2)}
                        className={`group relative w-full p-4 border rounded-lg text-left transition-all duration-300 ${
                          portableOUgTanks === 2 
                            ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50' 
                            : 'border-gray-300 hover:border-gray-500'
                    ...
                    >
                      <p className="font-medium text-[14px] leading-[20px] text-[#171A20]">2 Tanks</p>
                    </button>
                  </div>
                </div>
                ...
                <div className="mb-[45px]">
                  ...
                  <div className="space-y-4">
                    <button 
                        onClick={() => setPortableODuQuantity?.(portableODuQuantity === 1 ? 0 : 1)}
                        className={`group relative w-full p-4 border rounded-lg text-left transition-all duration-300 ${
                          portableODuQuantity === 1 
                            ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50' 
                            : 'border-gray-300 hover:border-gray-500'
                        }`}
                    >
                      <p className="font-medium text-[14px] leading-[20px] text-[#171A20]">1 DU</p>
                    </button>
                    <button 
                        onClick={() => setPortableODuQuantity?.(portableODuQuantity === 2 ? 0 : 2)}
                        className={`group relative w-full p-4 border rounded-lg text-left transition-all duration-300 ${
                          portableODuQuantity === 2 
                            ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50' 
                            : 'border-gray-300 hover:border-gray-500'
                        }`}
                    >
                      <p className="font-medium text-[14px] leading-[20px] text-[#171A20]">2 DU</p>
                    </button>
                    <button 
                        onClick={() => setPortableODuQuantity?.(portableODuQuantity === 3 ? 0 : 3)}
                        className={`group relative w-full p-4 border rounded-lg text-left transition-all duration-300 ${
                          portableODuQuantity === 3 
                            ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50' 
                            : 'border-gray-300 hover:border-gray-500'
                        }`}
                    >
                      <p className="font-medium text-[14px] leading-[20px] text-[#171A20]">3 DU</p>
                    </button>
                  </div>
                </div>
                ...
                {/* Technology */}
                <div className="mb-[45px]">
                  <h2 className="font-medium text-[20px] leading-[28px] text-[#171A20] mb-6 text-center">Technology</h2>
                  
                  {/* Fuel Monitoring System */}
                  <h3 className="font-medium text-[16px] leading-[24px] text-[#171A20] mb-3">Fuel Monitoring System</h3>
                  <div className="space-y-4 mb-6">
                    <div className="group w-full flex items-center justify-between p-4 border border-gray-300 rounded-lg bg-white">
                      <div className="flex items-center">
                        <p className="font-medium text-[14px] leading-[20px] text-[#171A20]">Fuel Level Sensors</p>
                      </div>
                      <div className="flex items-center gap-4">
                        {showPrices && <span className="text-[14px] text-gray-500">{formatPrice(89999 / 36)} / unit</span>}
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() => {
                              if (portableOFuelLevelSensors && portableOFuelLevelSensors > 0) {
                                setPortableOFuelLevelSensors?.(portableOFuelLevelSensors - 1);
                              }
                            }}
                            className="w-8 h-8 flex items-center justify-center rounded-full border border-gray-300 hover:bg-gray-50 text-gray-600 font-medium"
                          >-</button>
                          <span className="w-4 text-center text-sm font-medium">{portableOFuelLevelSensors || 0}</span>
                          <button
                            onClick={() => setPortableOFuelLevelSensors?.((portableOFuelLevelSensors || 0) + 1)}
                            className="w-8 h-8 flex items-center justify-center rounded-full border border-gray-300 hover:bg-gray-50 text-gray-600 font-medium"
                          >+</button>
                        </div>
                      </div>
                  ...
                  {/* Secure Fueling System */}
                  <h3 className="font-medium text-[16px] leading-[24px] text-[#171A20] mb-3">Secure Fueling System</h3>
                  <div className="space-y-4 mb-6">
                    <div className="group w-full flex items-center justify-between p-4 border border-gray-300 rounded-lg bg-white">
                      <div className="flex flex-col">
                        <p className="font-medium text-[14px] leading-[20px] text-[#171A20]">RFID</p>
                        <p className="text-[12px] text-gray-500">Active Reader</p>
                      </div>
                      <div className="flex items-center gap-4">
                        {showPrices && <span className="text-[14px] text-gray-500">{formatPrice(79999 / 36)} / unit</span>}
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() => {
                              if (portableORfidActive && portableORfidActive > 0) {
                                setPortableORfidActive?.(portableORfidActive - 1);
                              }
                            }}
                            className="w-8 h-8 flex items-center justify-center rounded-full border border-gray-300 hover:bg-gray-50 text-gray-600 font-medium"
                          >-</button>
                          <span className="w-4 text-center text-sm font-medium">{portableORfidActive || 0}</span>
                          <button
                            onClick={() => setPortableORfidActive?.((portableORfidActive || 0) + 1)}
                            className="w-8 h-8 flex items-center justify-center rounded-full border border-gray-300 hover:bg-gray-50 text-gray-600 font-medium"
                          >+</button>
                        </div>
                      </div>
                    ...
                    <div className="group w-full flex items-center justify-between p-4 border border-gray-300 rounded-lg bg-white">
                      <div className="flex flex-col">
                        <p className="font-medium text-[14px] leading-[20px] text-[#171A20]">RFID</p>
                        <p className="text-[12px] text-gray-500">Passive Tags</p>
                      </div>
                      <div className="flex items-center gap-4">
                        {showPrices && <span className="text-[14px] text-gray-500">{formatPrice(999 / 36)} / tag</span>}
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() => {
                              if (portableORfidPassive && portableORfidPassive > 0) {
                                setPortableORfidPassive?.(portableORfidPassive - 1);
                              }
                            }}
                            className="w-8 h-8 flex items-center justify-center rounded-full border border-gray-300 hover:bg-gray-50 text-gray-600 font-medium"
                          >-</button>
                          <span className="w-4 text-center text-sm font-medium">{portableORfidPassive || 0}</span>
                          <button
                            onClick={() => setPortableORfidPassive?.((portableORfidPassive || 0) + 1)}
                            className="w-8 h-8 flex items-center justify-center rounded-full border border-gray-300 hover:bg-gray-50 text-gray-600 font-medium"
                          >+</button>
                        </div>
                      </div>
                  ...
                  {/* OneSync */}
                  <h3 className="font-medium text-[16px] leading-[24px] text-[#171A20] mb-3">OneSync</h3>
                  <div className="space-y-4 mb-6">
                    <button
                      onClick={() => setPortableOIotController?.(!portableOIotController)}
                      className={`group w-full flex items-center justify-between p-4 border rounded-lg text-left cursor-pointer transition-all duration-300 ${
                        portableOIotController ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50' : 'border-gray-300 hover:border-gray-500'
                      }`}
                    >
                      <div className="flex items-center">
                        <div className={`h-5 w-5 border rounded flex-shrink-0 flex items-center justify-center transition-colors mr-3 ${
                          portableOIotController ? 'bg-gray-600 border-gray-600' : 'bg-white border-gray-300'
                        }`}>
                          {portableOIotController && (
                            <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="4">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                            </svg>
                          )}
                        </div>
                        <p className="font-medium text-[14px] leading-[20px] text-[#171A20]">IOT Controller</p>
                      </div>
                      {showPrices && <span className="text-[14px] text-gray-500">{formatPrice(99999 / 36)}</span>}
                    </button>
                  </div>

                  {/* Add ons */}
                  <h3 className="font-medium text-[16px] leading-[24px] text-[#171A20] mb-3">Add ons</h3>
                  <div className="space-y-4">
                    <button
                      onClick={() => setPortableODatumF?.(!portableODatumF)}
                      className={`group w-full flex items-center justify-between p-4 border rounded-lg text-left cursor-pointer transition-all duration-300 ${
                        portableODatumF ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50' : 'border-gray-300 hover:border-gray-500'
                      }`}
                    >
                      <div className="flex items-center">
                        <div className={`h-5 w-5 border rounded flex-shrink-0 flex items-center justify-center transition-colors mr-3 ${
                          portableODatumF ? 'bg-gray-600 border-gray-600' : 'bg-white border-gray-300'
                        }`}>
                          {portableODatumF && (
                            <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="4">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        ...
                        <p className="font-medium text-[14px] leading-[20px] text-[#171A20]">DATUM F</p>
                      </div>
                      {showPrices && <span className="text-[14px] text-gray-500">{formatPrice(899999 / 36)}</span>}
                    </button>

                    <button
                      onClick={() => setPortableOInstallation?.(!portableOInstallation)}
                      className={`group w-full flex items-center justify-between p-4 border rounded-lg text-left cursor-pointer transition-all duration-300 ${
                        portableOInstallation ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50' : 'border-gray-300 hover:border-gray-500'
                      }`}
                    >
                      <div className="flex items-center">
                        <div className={`h-5 w-5 border rounded flex-shrink-0 flex items-center justify-center transition-colors mr-3 ${
                          portableOInstallation ? 'bg-gray-600 border-gray-600' : 'bg-white border-gray-300'
                        }`}>
                          {portableOInstallation && (
                            <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="4">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                            </svg>
                          )}
                      ...
                      {showPrices && <span className="text-[14px] text-gray-500">{formatPrice(49999 / 36)}</span>}
                    </button>
                  </div>
                  
                  {/* DU Brand & Nozzle Selection */}
                  <div className="mt-6 mb-[45px]">
                    <h3 className="font-medium text-[16px] leading-[24px] text-[#171A20] mb-3">DU Brand & Nozzle Selection</h3>
                    <div className="space-y-4">
                      <div className="flex flex-col gap-2">
                        <label className="text-sm text-gray-600 font-medium">Select DU Brand</label>
                        <select 
                          className="p-3 border border-gray-300 rounded-lg text-sm bg-white"
                          value={portableODuBrand || ''}
                          onChange={(e) => {
                             setPortableODuBrand?.(e.target.value);
                             if (!portableODuNozzle) {
                               setPortableODuNozzle?.('Single Nozzle');
                             }
                          }}
                        >
                      ...
                      {portableODuBrand && (
                        <div className="flex flex-col gap-2">
                          <label className="text-sm text-gray-600 font-medium">Select Nozzle Configuration</label>
                          <div className="grid grid-cols-2 gap-3">
                            <button
                              onClick={() => setPortableODuNozzle?.('Single Nozzle')}
                              className={`group w-full flex flex-col items-center justify-center p-3 border rounded-lg text-center cursor-pointer transition-all duration-300 ${
                                portableODuNozzle === 'Single Nozzle' ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50' : 'border-gray-300 hover:border-gray-500'
                              }`}
                            >
                              <p className="font-medium text-[14px] text-[#171A20]">Single Nozzle</p>
                              {showPrices && <span className="text-[12px] text-gray-500 mt-1">{formatPrice(129999 / 36)}</span>}
                            </button>
                            <button
                              onClick={() => setPortableODuNozzle?.('Double Nozzle')}
                              className={`group w-full flex flex-col items-center justify-center p-3 border rounded-lg text-center cursor-pointer transition-all duration-300 ${
                                portableODuNozzle === 'Double Nozzle' ? 'border-gray-400 ring-1 ring-gray-400 bg-gray-50' : 'border-gray-300 hover:border-gray-500'
                              }`}
                            >
                              <p className="font-medium text-[14px] text-[#171A20]">Double Nozzle</p>
                              {showPrices && <span className="text-[12px] text-gray-500 mt-1">{formatPrice(249999 / 36)}</span>}
                            </button>
                          </div>
                        </div>
                      )}
            ...
            {showPrices ? (
              <div ref={pricingSectionRef} className="pt-8 border-t border-gray-200">
                <div className="flex justify-between items-center mb-6 cursor-pointer group select-none" onClick={() => setIsPricingDetailsOpen(!isPricingDetailsOpen)}>
                  ...
                  <div className="flex items-center gap-4">
                      {selectedProduct !== 'portable-o' && selectedProduct !== 'rfd-z' && (
              ...
              <div className="pt-8 border-t border-gray-200 mb-8">
                  <div className="flex gap-4 flex-col sm:flex-row">
                     <button className="flex-1 py-3 px-4 rounded text-sm font-semibold bg-gray-100 text-gray-800 transition-colors hover:bg-gray-200" onClick={onCalculateRoi}>Calculate ROI</button>
                     <button className="flex-1 py-3 px-4 rounded text-sm font-semibold bg-blue-600 text-white transition-colors hover:bg-blue-700" onClick={onOpenQuote}>View Quote</button>
                  </div>
              </div>
            )}
          </div>
                ...
                <div>
                  ...
                  {paymentMode === 'installments' ? (
                    <div className="flex items-center gap-1.5 mt-0.5 text-xs text-gray-500">
                      <span>Per Month</span>
                      <span>•</span>
                      <span>GST @18% additional</span>
                    </div>
                  ) : (
                    <p className="text-xs text-gray-500 mt-0.5">GST @18% additional</p>
                  )}
                </div>
