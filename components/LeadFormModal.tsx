...
const LeadFormModal: React.FC<LeadFormModalProps> = ({ onSubmit, loggedInUser, initialData }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[LeadFormModal] Lead submission failed:', err);
      let userFriendlyMessage = "Failed to submit lead. Please try again.";
      if (err?.message) {
        try {
          const parsed = JSON.parse(err.message);
          if (parsed.error) {
            userFriendlyMessage = `Submission error: ${parsed.error}`;
          }
        } catch {
