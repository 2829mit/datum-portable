...
const App: React.FC = () => {
  ...
  const [selectedProduct, setSelectedProduct] = useState<'rfd-portable' | 'rfd-z' | 'portable-o'>('rfd-portable');

  // Portable O states
  const [portableOUgTanks, setPortableOUgTanks] = useState<0 | 1 | 2>(0);
  const [portableODuQuantity, setPortableODuQuantity] = useState<0 | 1 | 2 | 3>(0);
  const [portableOFuelLevelSensors, setPortableOFuelLevelSensors] = useState<number>(0);
  const [portableORfidActive, setPortableORfidActive] = useState<number>(0);
  const [portableORfidPassive, setPortableORfidPassive] = useState<number>(0);
  const [portableOIotController, setPortableOIotController] = useState<boolean>(false);
  const [portableODatumF, setPortableODatumF] = useState<boolean>(false);
  const [portableOInstallation, setPortableOInstallation] = useState<boolean>(false);
  const [portableODuBrand, setPortableODuBrand] = useState<string>('');
  const [portableODuNozzle, setPortableODuNozzle] = useState<string>('');
  ...
  useEffect(() => {
    if (selectedProduct === 'rfd-z' || selectedProduct === 'portable-o') {
...
const handleSafetyUpgradeToggle = (option: SafetyUpgradeOption) => {
  ...
  const resetConfiguration = () => {
    ...
    // Portable O state
    setPortableOUgTanks(0);
    setPortableODuQuantity(0);
    setPortableOFuelLevelSensors(0);
    setPortableORfidActive(0);
    setPortableORfidPassive(0);
    setPortableOIotController(false);
    setPortableODatumF(false);
    setPortableOInstallation(false);
    setPortableODuBrand('');
  ...
  const monthlyTotalPrice = useMemo(() => {
    if (selectedProduct === 'portable-o') {
      let price = 0;
      price += (portableOFuelLevelSensors * (89999 / 36));
      price += (portableORfidActive * (79999 / 36));
      price += (portableORfidPassive * (999 / 36));
      if (portableOIotController) price += (99999 / 36);
      if (portableODatumF) price += (899999 / 36);
      if (portableOInstallation) price += (49999 / 36);
      
      if (portableODuNozzle === 'Single Nozzle') {
        price += (129999 / 36);
      } else if (portableODuNozzle === 'Double Nozzle') {
        price += (249999 / 36);
      }
      return price;
    ...
    rfdZBase,
    rfdZUpgrades,
    portableOFuelLevelSensors,
    portableORfidActive,
    portableORfidPassive,
    ...
    if (details) {
      ...
      if (pendingQuoteGeneration) {
        const quoteData: QuoteData = {
          ...
          portableOConfiguration: selectedProduct === 'portable-o' ? {
            ugTanks: portableOUgTanks,
            duQuantity: portableODuQuantity,
            fuelLevelSensors: portableOFuelLevelSensors,
            rfidActive: portableORfidActive,
            rfidPassive: portableORfidPassive,
            iotController: portableOIotController,
            datumF: portableODatumF,
            installation: portableOInstallation,
            duBrand: portableODuBrand,
            duNozzle: portableODuNozzle,
          } : undefined,
          configuration: {
            tank: selectedTank,
        ...
        <main className="flex flex-col lg:flex-row">
          <div className="w-full lg:flex-1 lg:h-[calc(100vh-72px)] lg:sticky lg:top-[72px] h-[45vh] min-h-[300px] relative z-0 bg-gray-100">
            <CarVisualizer 
              ...
              portableOUgTanks={portableOUgTanks}
              portableODuQuantity={portableODuQuantity}
              portableOFuelLevelSensors={portableOFuelLevelSensors}
              portableORfidActive={portableORfidActive}
              portableORfidPassive={portableORfidPassive}
              portableOIotController={portableOIotController}
              portableODatumF={portableODatumF}
            />
          </div>
          <div className="w-full lg:w-[400px] xl:w-[450px] lg:h-[calc(100vh-72px)] bg-white z-10 flex-shrink-0">
            <Configurator
              ...
              portableOUgTanks={portableOUgTanks}
              setPortableOUgTanks={setPortableOUgTanks}
              portableODuQuantity={portableODuQuantity}
              setPortableODuQuantity={setPortableODuQuantity}
              portableOFuelLevelSensors={portableOFuelLevelSensors}
              setPortableOFuelLevelSensors={setPortableOFuelLevelSensors}
              portableORfidActive={portableORfidActive}
              setPortableORfidActive={setPortableORfidActive}
              portableORfidPassive={portableORfidPassive}
              setPortableORfidPassive={setPortableORfidPassive}
              portableOIotController={portableOIotController}
              setPortableOIotController={setPortableOIotController}
              portableODatumF={portableODatumF}
              setPortableODatumF={setPortableODatumF}
              portableOInstallation={portableOInstallation}
              setPortableOInstallation={setPortableOInstallation}
              portableODuBrand={portableODuBrand}
              setPortableODuBrand={setPortableODuBrand}
              portableODuNozzle={portableODuNozzle}
              setPortableODuNozzle={setPortableODuNozzle}
            />
          </div>
        </main>
      )}
