...
import { TANK_OPTIONS, RFD_Z_BASE_OPTIONS, RFD_Z_UPGRADE_OPTIONS } from '../constants';
...
export const generateQuotePDF = async (data: QuoteData) => {
  ...
  yPos += 3.5;
  doc.text("Office No 103, First Floor, Sr No 209/1/1-A, Kumar Vastu Co-Op Housing Society,", 105, yPos, { align: "center" });
  yPos += 3.5;
  doc.text("Range Hills Road, Opp Symphony Hotel, Range Hills, Bhosale Nagar, Shivaji Nagar,", 105, yPos, { align: "center" });
  yPos += 3.5;
  doc.text("Pune - 411020, Maharashtra", 105, yPos, { align: "center" });
  yPos += 3.5;
  ...
  const OPTION_SUB_POINTS: Record<string, string[]> = {
    // Only options that have sub-components explicitly displayed in the UI:
    // 1. Safety & Security System (UI flap displays: Crash Barrier, Fire Suppression, 360° Camera)
    'safety-security-system': [
      'Crash Barrier',
      'Fire Suppression',
      '360° Camera'
    ],
    // 2. Licenses and Compliances (UI flap displays: PESO Drawing Creation, PESO Prior Approval & Site Inspection, PESO final Grant - Form XIV, Tank & Nozzle Calibration)
    'licenses-compliances': [
      'PESO Drawing Creation',
      'PESO Prior Approval & Site Inspection',
      'PESO final Grant - Form XIV',
      'Tank & Nozzle Calibration'
    ]
  };

  const allSelectedItems: LineItem[] = [];
  
  if (data.selectedProduct === 'portable-o') {
    const pConf = data.portableOConfiguration;
    if (pConf) {
       const standardInclusions: string[] = [];
       const addonItems: LineItem[] = [];

       if (pConf.ugTanks > 0) {
         standardInclusions.push(`Underground Tanks (${pConf.ugTanks} nos)`);
       }
       if (pConf.duQuantity > 0) {
         standardInclusions.push(`Dispensing Unit (${pConf.duQuantity} nos)`);
       }
       
       if (pConf.fuelLevelSensors > 0) {
         addonItems.push({
           desc: `Add-On: Fuel Level Sensors`,
           rate: (89999 / 36) * multiplier,
           hsn: "84131191",
           tenure: isInstallment ? "36" : "Nos",
           isAddon: true,
           quantity: pConf.fuelLevelSensors.toString()
         });
       }
       if (pConf.rfidActive > 0) {
         addonItems.push({
           desc: `Add-On: RFID (Active Reader)`,
           rate: (79999 / 36) * multiplier,
           hsn: "84131191",
           tenure: isInstallment ? "36" : "Nos",
           isAddon: true,
           quantity: pConf.rfidActive.toString()
         });
       }
       if (pConf.rfidPassive > 0) {
         addonItems.push({
           desc: `Add-On: RFID (Passive Tags)`,
           rate: (999 / 36) * multiplier,
           hsn: "84131191",
           tenure: isInstallment ? "36" : "Nos",
           isAddon: true,
           quantity: pConf.rfidPassive.toString()
         });
       }
       if (pConf.iotController) {
         addonItems.push({
           desc: `Add-On: IOT Controller`,
           rate: (99999 / 36) * multiplier,
           hsn: "84131191",
           tenure: isInstallment ? "36" : "Nos",
           isAddon: true,
           quantity: "1"
       ...
       if (pConf.installation) {
         addonItems.push({
           desc: `Add-On: Installation & Commissioning`,
           rate: (49999 / 36) * multiplier,
           hsn: "84131191",
           tenure: isInstallment ? "36" : "Nos",
           isAddon: true,
           quantity: "1"
       ...
       if (pConf.duBrand && pConf.duNozzle) {
         let p = 0;
         if (pConf.duNozzle === 'Single Nozzle') p = 129999;
         if (pConf.duNozzle === 'Double Nozzle') p = 249999;
         addonItems.push({
           desc: `Add-On: DU Brand (${pConf.duBrand}) - ${pConf.duNozzle}`,
           rate: (p / 36) * multiplier,
           hsn: "84131191",
           tenure: isInstallment ? "36" : "Nos",
           isAddon: true,
           quantity: "1"
       ...
       let mainDesc = "Repos Portable O";
       if (standardInclusions.length > 0) {
         mainDesc += `\n\nStandard Inclusions:\n` + standardInclusions.map(name => `• ${name}`).join('\n');
       }
       ...
       addonItems.forEach(item => allSelectedItems.push(item));
    }
  } else if (data.selectedProduct === 'rfd-z') {
    ...
    if (data.rfdZConfiguration?.upgrades?.length) {
      data.rfdZConfiguration.upgrades.forEach(upgradeId => {
        ...
        const subPoints = OPTION_SUB_POINTS[upgradeId] || [];
        if (price === 0) {
          standardInclusions.push(name);
        } else {
          let desc = `Add-On: ${name}`;
    ...
    let mainDesc = `Repos RFD Z Base Unit: ${baseOpt ? baseOpt.name : 'Datum 2KL'}`;
    if (standardInclusions.length > 0) {
      mainDesc += `\n\nStandard Inclusions:\n` + standardInclusions.map(name => `• ${name}`).join('\n');
    }
    ...
    const collectItems = (list: any[]) => {
        list.forEach(item => {
            ...
            if (item.id === 'single-du') {
              description = "Single DU (2 Nozzle with Active Readers)";
            }

            if (item.id === 'advanced-skid') {
              description = "Advanced Decanting Skid";
            ...
            if (item.id === 'secure-fueling-2') {
              description = "Secure Fuelling System (2-Active Tag)";
            }

            if (item.id === 'basic-skid-dispensing') {
              description = "Basic Decanting Skid";
            }

            if (item.id === 'basic-skid') {
              description = "Basic Decanting Skid (Without Metering Counter)";
            }

            if (item.id === 'reports-analytics') {
              description = "Repos OS: Reports & Analytics";
            }

            if (item.id === 'operator-manager-apps') {
              description = "Repos OS: Operator & Manager Apps";
            }

            if (item.id === 'real-time-inventory') {
              description = "Repos OS: Real-time Fuel Level Inventory";
            }
            
            if (rate === 0) {
              standardInclusions.push(description);
            } else {
              const subPoints = OPTION_SUB_POINTS[item.id] || [];
              let desc = `Add-On: ${description}`;
              if (subPoints.length > 0) {
                desc += '\n' + subPoints.map(p => `• ${p}`).join('\n');
              }
              addonItems.push({
    ...
    if (data.rfidTagsQuantity && data.rfidTagsQuantity > 0) {
      addonItems.push({
        desc: `Add-On: RFID Tags (${data.rfidTagsQuantity} nos)`,
        rate: (data.rfidTagsQuantity * (999 / 36)) * multiplier,
        hsn: "84131191",
        tenure: isInstallment ? "36" : "Nos",
        isAddon: true,
    ...
    // Sr. No. 1: Base Product with all standard inclusions listed inside
    allSelectedItems.push({
  ...
  // Representative Name text at the bottom of the signature cell
  const repName = data.customerDetails?.salesperson ? data.customerDetails.salesperson : "Name of Repos Representative";
  doc.text(repName, 170, pageHeight - 10, { align: "center" });
